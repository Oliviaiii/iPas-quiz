...
test("keeps explanation drafts complete and free of filler", () => {
  // 第二階段逐批撰寫；每批完成後更新此數字。
  // 114 年第四次初級兩科、115 年第一次初級兩科、115 年第二次初級第一科完成。
  const draftIds = [
    ...
    ...Array.from({ length: 50 }, (_, index) =>
      `aiap-elementary-115-01-ai-foundation-${String(index + 1).padStart(3, "0")}`,
    ),
    ...Array.from({ length: 50 }, (_, index) =>
      `aiap-elementary-115-01-genai-planning-${String(index + 1).padStart(3, "0")}`,
    ),
    ...Array.from({ length: 50 }, (_, index) =>
      `aiap-elementary-115-02-ai-foundation-${String(index + 1).padStart(3, "0")}`,
    ),
  ];
  assert.deepEqual(
    questions
...
test("publishes the verified inventory and imported-question totals", () => {
  ...
  assert.equal(manifest.explanationStatus.missing, 350);
  assert.equal(manifest.explanationStatus.draft, 250);
