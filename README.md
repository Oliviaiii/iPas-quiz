...
2026-07-31，可確認的匯入目標為 12 份官方歷屆試卷、共 600 題；原盤點的
115 題官方樣題已下架，不計入目前目標。
...
- 中級 115 年第一次：上述三科各 50 題。
- 題目與答案已逐頁對照官方 PDF。
- 點選選項後會鎖定答案並顯示答對／答錯及正確答案；已有實質內容的題目會
  另外顯示 A～D 選項解析。
- 可依科目篩選、只看錯題，作答進度保存在瀏覽器本機。

本站詳解是 AI 輔助撰寫的初稿，不是官方詳解。目前固定題庫 600 題皆已有完整
A～D 解析初稿；全部仍待不同於撰寫者的獨立人工複核，不使用模板句假裝詳解或
複核已完成。原有 4 題非官方介面示範題已全部移除，不列入題庫及統計。
