...
## 2026-08-31 全站排版掃描：中英數字之間補空格（246 處、161 題）

- 處理範圍：前一份交接列為未解問題 2 的跨題排版瑕疵——詳解內文的中文與半形英數字
  之間漏空格（交接文件舉的例子「只有 A直接對應」正是其中一處，已修）。
- **先量再改**：全庫掃描顯示已加空格的邊界有 17,086 處、漏空格的只有 249 處，
  既有慣例明確，因此這是把少數不一致者向多數對齊，不是新訂風格。
- 結果：補上 246 個半形空格，涵蓋 161 題；刻意保留 3 處不加：
  - `星期一_08時`、`星期日_08時`（114-4 第一科 Q2）：特徵交叉的識別字字面值，
    加空格會改變這個「名稱」本身。
  - `w1·L分類`（114-2 中級第一科 Q47）：下標式公式，`L 分類` 反而更難讀。
- **範圍限制**：只動自撰的詳解散文（`summary`、`concept`、`answerReason`、
  `optionAnalysis.A`～`D`、`trap`、`editorialNote`）。`prompt`、`options`、`passage`、
  `figures` 是官方原文，一律不碰；`references` 的 title 與 locator 掃過後為零筆
  待修，本來就合乎慣例，也未更動。
- 腳本：`scripts/fix-typography-cjk-latin-spacing.py`。防護條件三道：
  1. 全庫 sha256 摘要須符合掃描當時狀態；
  2. 實際插入的空格數須等於掃描算出的 246；
  3. **關鍵防護——把新舊字串的所有空格去掉後必須完全相同**，
     這在機制上保證腳本只可能插入空格，不可能改到任何一個字。
  另有兩條略過規則：反引號程式碼片段內部，以及「底線直接與中文相鄰」的混合識別字。
- 事後驗證：重新掃描僅剩上述 3 處刻意例外；逐題比對確認 `prompt`／`options`／
  `officialAnswer`／`explanationStatus`／`figures`／`passage`／`references` 零變更，
  詳解欄位去空格後與原文逐字相同（violations: 0）；全庫亦無連續兩個空格或
  空格緊接全形標點的情形。
- 規則已寫入 `docs/QUESTION_AUTHORING_GUIDE.md` 新增的「6.1 中英夾雜的排版」一節，
  日後新寫詳解直接沿用，並可用同一支腳本重跑。
- 執行測試：`scripts/validate-ai-review-reports.py`（60 份報告 600 題全通過）、
  `npm test` 9／9；manifest 重生後仍為 `draft: 600`／`missing: 0`／`reviewed: 0`。
- 未解問題（更新後）：只剩 **20 題 human-decision 待人類裁決**，清單見各批次報告的
  `findings`。開放待查項目 0 題、`blocked` 0 題、排版掃描已完成。
- 下一步：第二階段 B 的人工複核。

## 2026-08-31 選項分析／核心觀念／解題理由白話化（進行中，因額度暫停）

- 起因：使用者非本科系，回報選項分析「內容看得懂但用語看不懂」。目標是在
  **不刪專有名詞、不改任何技術判斷**的前提下補上白話說明。範圍經使用者確認為
  `optionAnalysis`、`concept`、`answerReason` 三欄，共 3,600 個欄位。
- **本次做到 240 欄，其餘 3,360 欄未動。額度考量下已停止全部 12 個 agent。**

### 已完成並套用（240 欄）

| 範圍 | 欄位 | 數量 |
| --- | --- | --- |
| 114-4 初級第一科 Q1～50 | optionAnalysis | 200 |
| 114-4 初級第一科 Q1～10 | concept／answerReason | 20 |

payload 存於 `content/plain-language/`，每筆都含被取代原文與其 sha256，
改寫前後對照永久查得到。

### 未完成（3,360 欄）

- 114-4 初級第一科 Q11～50 的 concept／answerReason（80 欄）
- 其餘 11 份試卷的三欄全部（3,280 欄）

### 恢復工作時必讀

1. **規則在 `docs/PLAIN_LANGUAGE_GUIDE.md`**，其中 **3.1 節是重點**。第一輪 12 個
   agent 全部踩到同一個坑：把原句留著、旁邊掛一個註解或比喻就交差。那是加註解，
   不是白話化，讀者一樣在第十個字卡住。3.1 節用三組實際改稿寫清楚了差別，並列出
   映射、樣態、表徵、粒度、封裝等出現時必須換掉的詞。判準是「念給沒學過 AI 的人聽，
   他會不會停下來問這是什麼意思」。
2. **舊標準的 7 份 payload 已刪除**（114-4 第二科、115-1 第一科、115-2 第一科
   Q26～50、115-2 第二科），它們是踩到 3.1 節那個坑的半成品，重做時直接從題庫現況
   重新產生即可，不要沿用。
3. 流程：改寫者只產 payload，**不得直接改 `app/data/questions.json`**；
   由主流程以 `scripts/apply-plain-language.py` 套用，再以
   `scripts/check-plain-language.py` 驗收。
4. 一個 agent 負責 25～50 題較合適；scratchpad 檔名要各自唯一，先前發生過兩個
   agent 互相覆蓋暫存檔的情形。

### 三道自動防護（都在腳本裡，不靠人記得）

- 舊文 sha256 必須相符——payload 無法重複套用，也蓋不掉他人的修改。
- 「正確。」標記不得被拿掉，也不得加到非答案選項上。
- **名詞與數字保留**：原文出現過的英文術語、法條號、百分比，改寫後必須全都還在。
  這是唯一能自動抓到「把內容刪掉當成簡化」的檢查，本次已抓到兩次真實漏失
  （`AI 系統` 被寫成 `系統`、`API Token` 被寫成 `token`）。

### 順帶完成的兩項

- **前端修正**：600 題中有 297 題的 `concept` 以換行分段，但 CSS 未設
  `white-space: pre-line`，瀏覽器把換行摺成空格，作者寫的段落結構在畫面上整個消失。
  已補上，分段現在顯示得出來。這對可讀性的幫助不比改字小。
- **改寫時發現一個真正的內容錯誤**：114-4 初級第二科第 7 題選項 A 的解析，
  原以「此順序把 Host 排在 Client 之前」作為 A 的缺失，但官方答案 C 也是
  Host 在 Client 之前，這不可能是 A 的錯處。已修正為方向問題（A 由外往內，
  與 Client 內嵌於 Host、請求由內往外的架構相反），並回填批次報告，該題改判
  `corrected`。600 題複核當時沒抓到，白話化反而逼出來——因為要講成人話就得真的想通。
...
1. **114-2 中級二 Q43 選項 A 的「正確。」是原文的錯誤標記。**
   該題官方答案是 (B)，選項 (A)「原因 B、原因 C」是錯的，但原稿解析開頭卻寫
   「正確。原因 B 與 C 都不成立」——說它正確，又解釋它為什麼不成立，自相矛盾。
   白話化 payload 已改寫為「正確答案不是這一組。原因 B 與原因 C 都不成立……」，
   套用後矛盾即消失。**套用時應把這一題回填到該批次報告，改判 corrected。**
   （這是 2026-08-31 排版掃描時就掃出的 MISMATCH 之一，當時未追查。）

2. **115-1 中級三 Q46 的「低秩近似」被改寫成「用少數幾個方向去近似重建」。**
   該詞是純中文，名詞保留檢查抓不到。規則要的是「名詞＋白話」並存，套用前
   應把「低秩近似」補回去。

3. **兩處原文連寫錯字，改寫時刻意照原樣重現、未默默修掉**：
