...
the payload is written to ``…-prose.json`` so the passes never collide::

    {"11": {"concept": "…", "answerReason": "…"}, "12": {…}}

``--trap`` is the same shape for the single ``trap`` field, written to
``…-trap.json``::
...
def main() -> None:
    ...
    prose = "--prose" in sys.argv or trap
    if trap and "--prose" in sys.argv:
        raise SystemExit("--prose 與 --trap 不能同時使用")
    fields = ("trap",) if trap else PROSE_FIELDS
    if len(argv) != 4:
    ...
    for number in numbers:
        ...
        for field, new in draft[str(number)].items():
            if prose and field not in fields:
                raise SystemExit(f"Q{number}: {field} is not a prose field {fields}")
            ...
            total += 1
        expected = set(fields) if prose else set(source)
    ...
    suffix = "-trap" if trap else ("-prose" if prose else "")
    ...
    kind = "trap fields" if trap else ("prose fields" if prose else "option analyses")
